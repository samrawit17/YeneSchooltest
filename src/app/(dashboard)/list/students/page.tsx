...
import { studentsAPI, bulkUploadAPI, classesAPI, sectionsAPI } from "@/lib/api";
...
const StudentsListPage = () => {
  ...
  // Class and Section filter options
    ...
    const fetchFilterOptions = async () => {
      try {
        ...
        if (classesResponse.data) {
          const classes = classesResponse.data.map((cls: any) => ({
            id: cls.id,
        ...
        if (sectionsResponse.data) {
          const sections = sectionsResponse.data.map((sec: any) => ({
            id: sec.id,
            name: sec.name
          }));
      ...
      <div className="p-4 md:p-6">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <h1 className="text-2xl font-bold text-[#e35336]">Students</h1>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  >
                    ...
                    {classOptions.map((cls) => (
                      <option key={cls.id} value={cls.grade}>{cls.name}</option>
                    ))}
                  </select>
