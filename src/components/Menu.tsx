...
const menuItems: MenuSection[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        label: "Messages",
        href: "/messages",
        visible: ["teacher", "admin", "registrar", "finance", "hr"],
        featureFlag: "FEATURE_FLAG_CHAT",
      },
      {
        icon: <Users className="w-5 h-5" />,
        label: "Students",
      ...
      },
      {
        ...
        href: "/list/finance",
        visible: ["finance"],
        featureFlag: "FEATURE_FLAG_PAYMENTS",
            ...
            if (isVisible) {
              return (
                <Link
                  ...
                  onClick={() => onItemClick?.()}
                  className={`flex items-center justify-start gap-3 py-3 px-4 rounded-lg transition-colors relative ${isActive
                    ? "bg-blue-100 dark:bg-[#1E293B] text-[#e35336] dark:text-white font-medium"
                    : "text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-[#1E293B]"
                    } ${collapsed ? 'justify-center' : ''}`}
                  title={collapsed ? item.label : undefined}
                >
                  <div className={`relative ${isActive ? "text-[#e35336] dark:text-white" : "text-gray-600 dark:text-gray-300"}`}>
                    {item.icon}
                    {(showBadge || showEventBadge) && collapsed && (
                      <span className="absolute top-0 right-0 transform translate-x-1/3 -translate-y-1/3 bg-red-500 text-white text-[10px] font-bold min-w-[16px] h-4 rounded-full flex items-center justify-center px-1">
                        {showCommBadge ? (openCommunicationsCount > 9 ? '9+' : openCommunicationsCount) :
                          showAnnouncementBadge ? (announcementsCount > 9 ? '9+' : announcementsCount) :
                            (eventsCount > 9 ? '9+' : eventsCount)}
                      </span>
                    )}
                  </div>
                  <span className={`text-sm ${collapsed ? 'hidden' : ''} ${isActive ? "font-medium" : ""
                    }`}>
                    {displayLabel}
                  </span>
                  {showBadge && !collapsed && (
                    <span className="ml-auto bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full min-w-[20px] text-center">
                      {showCommBadge ? (openCommunicationsCount > 99 ? '99+' : openCommunicationsCount) :
                        showAnnouncementBadge ? (announcementsCount > 99 ? '99+' : announcementsCount) :
