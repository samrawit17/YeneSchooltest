      ...
      <div className="w-full space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[380px_1fr]">
          ...
          <Card className="border-gray-200 bg-white shadow-sm dark:border-[#2A2A2A] dark:bg-[#111111]">
            ...
            <CardContent>
              {children.length === 0 ? (
                <div className="rounded-xl border border-dashed border-gray-200 py-12 text-center dark:border-[#2A2A2A]">
                  <Users className="mx-auto mb-3 h-10 w-10 text-gray-300" />
                  <p className="font-medium text-gray-900 dark:text-white">No children linked</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Link a student to this parent profile.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4 2xl:grid-cols-2">
                  {children.map((child: any) => (
                    <Link
                      ...
                      className="block rounded-xl border border-gray-200 p-4 transition-colors hover:border-[var(--brand-color,#e35336)] hover:bg-[rgba(var(--brand-color-rgb),0.06)] dark:border-[#2A2A2A] dark:hover:bg-[rgba(var(--brand-color-rgb),0.12)]"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="truncate font-semibold text-gray-900 dark:text-white">{child.name}</p>
                          <p className="mt-1 font-mono text-sm text-gray-500 dark:text-gray-400">{child.studentCode || "N/A"}</p>
                        </div>
...
function InfoRow({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
  return (
    <div className="flex items-start gap-3 rounded-lg border border-gray-100 p-3 dark:border-[#2A2A2A]">
...
function MiniInfo({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="truncate font-medium text-gray-700 dark:text-gray-200">{value || "N/A"}</p>
    </div>
  );
}
