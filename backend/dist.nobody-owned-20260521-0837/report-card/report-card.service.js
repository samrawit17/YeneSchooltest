...
let ReportCardService = class ReportCardService {
    ...
    async generateCertificatePdf(schoolId, reportCardId) {
        ...
        const assessmentColumns = Array.from(new Map(grades
            .flatMap((grade) => Array.isArray(grade.assessmentBreakdown) ? grade.assessmentBreakdown : [])
            .map((item) => [String(item.type || item.title || 'Assessment').toUpperCase(), {
        ...
        assessmentColumns.forEach((column) => {
            drawCenteredText(truncate(column.label, assessmentW - 4, 8, bold), cursorX + assessmentW / 2, tableTop + 7, 8, bold, (0, pdf_lib_1.rgb)(1, 1, 1));
            cursorX += assessmentW;
        });
        drawCenteredText('Total', cursorX + totalW / 2, tableTop + 7, 9, bold, (0, pdf_lib_1.rgb)(1, 1, 1));
        cursorX += totalW;
        drawCenteredText('Grade', cursorX + gradeW / 2, tableTop + 7, 9, bold, (0, pdf_lib_1.rgb)(1, 1, 1));
        ...
        for (const [index, grade] of grades.slice(0, 14).entries()) {
            ...
            cursorX += subjectW;
            assessmentColumns.forEach((column) => {
                const match = (grade.assessmentBreakdown || []).find((item) => String(item.type || item.title || 'Assessment').toUpperCase() === column.key);
                drawCenteredText(match?.score ?? '-', cursorX + assessmentW / 2, rowY + 7, 8.5, font, darkText);
                cursorX += assessmentW;
            });
            drawCenteredText(String(grade.totalScore ?? '-'), cursorX + totalW / 2, rowY + 7, 9, bold, darkText);
            cursorX += totalW;
