...
let TimetableSlotController = class TimetableSlotController {
    ...
    async findByTeacher(req, targetTeacherId) {
        const schoolId = req.user.schoolId;
        if (!schoolId) {
            return { success: false, message: 'School ID is required' };
        }
        const resolvedTeacherId = await this.timetableSlotService.resolveTeacherTimetableTarget(schoolId, req.user, targetTeacherId);
        const result = await this.timetableSlotService.findByTeacher(schoolId, resolvedTeacherId);
        return result;
    }
    ...
    async deleteByClassSection(req, classId, sectionId) {
        const schoolId = req.user.schoolId;
        if (!schoolId) {
            return { success: false, message: 'School ID is required' };
        }
        return this.timetableSlotService.deleteByClassSection(schoolId, classId, sectionId);
    }
    async getTimetableGrid(req, classId, sectionId, academicYearId) {
        ...
        if (req.user.role === role_enum_1.Role.PARENT) {
            await this.timetableSlotService.assertParentCanViewClassTimetable(schoolId, req.user.id, classId, sectionId);
        }
