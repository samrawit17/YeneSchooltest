...
let BulkUploadService = class BulkUploadService {
    ...
    getNormalizedStudentName(record) {
        const joined = [record.first_name, record.middle_name, record.last_name]
            .filter(Boolean)
            .join(' ')
            .trim() ||
            record.full_name ||
    ...
    validateRecord(record, index) {
        ...
        const isStudent = record.role?.toLowerCase() === 'student' ||
    ...
    async processBulkStudentsWithAssignment(schoolId, uploadedById, records, academicYear) {
        ...
        if (academicYear && !requestedAcademicYear) {
            throw new common_1.BadRequestException('Selected academic year was not found for this school');
        }
        const yearId = selectedAcademicYear?.id;
        const yearName = selectedAcademicYear?.name;
        if (!yearId) {
            throw new common_1.BadRequestException('No academic year found for this school');
        }
        if (!yearName)
            throw new common_1.BadRequestException('No academic year name found');
