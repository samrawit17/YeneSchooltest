...
let StudentController = class StudentController {
    ...
    async getStudents(req, classId, sectionId, section, status, grade, page, limit, search, rollNumber) {
        const schoolId = req.user.schoolId;
        if (!schoolId) {
            throw new common_1.BadRequestException('School context is required');
        }
    ...
    async generateIdCardsBulkPdf(req, body, res) {
        const zip = await this.studentService.generateIdCardBulkZip(req.user.schoolId, body.studentIds || []);
    ...
    async getMyHomeroomStudents(req) {
        const schoolId = req.user.schoolId;
        const teacherId = req.user.id;
        const requesterRole = req.user.role;
        return this.studentService.getStudentsByHomeroomTeacher(schoolId, teacherId, requesterRole);
    }
