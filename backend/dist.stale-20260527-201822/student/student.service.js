...
let StudentService = class StudentService {
    ...
    classService;
    cacheService;
    constructor(prismaService, credentialService, classService, cacheService) {
    ...
    async createStudent(createStudentDto, createdById) {
        const { email, name, schoolId, academicYear, grade, className, section, rollNumber, gender, address, phone, motherName, motherPhone, emergencyContact, guardianName, guardianPhone, guardianEmail, photo, documents, } = createStudentDto;
    ...
    async generateIdCardPdf(schoolId, studentId) {
        ...
        if (!student)
            throw new common_1.NotFoundException('Student not found for ID card');
    ...
    async generateIdCardBulkZip(schoolId, studentIds) {
        ...
        if (!ids.length)
            throw new common_1.BadRequestException('No student IDs provided');
        const chunks = [];
    ...
    async updateStudent(studentId, schoolId, updateStudentDto) {
        ...
        const { name, gender, address, phone, motherName, motherPhone, emergencyContact, guardianName, guardianPhone, guardianEmail, photo, documents, } = updateStudentDto;
        if (name || photo) {
    ...
    async getStudentsByHomeroomTeacher(schoolId, teacherId, requesterRole) {
