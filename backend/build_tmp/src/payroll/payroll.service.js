...
let PayrollService = PayrollService_1 = class PayrollService {
    ...
    async listPayrollStaff(schoolId) {
        ...
        return users.map((user) => ({
            ...
            designation: user.teacherProfile?.designation ||
                (user.role === role_enum_1.Role.FINANCE ? 'Finance Staff' : null),
    ...
    async listPayrollSalaries(schoolId) {
    ...
    async upsertPayrollSalary(user, dto) {
        ...
        if (!staff) {
            throw new localization_1.LocalizedException('payroll.staff_member_not_found_for_this_school_b5cc1d91', undefined, common_1.HttpStatus.NOT_FOUND, 'Staff member not found for this school');
        }
    ...
    async listPayrollRuns(query) {
        const where = { schoolId: query.schoolId };
        if (query.month)
        ...
        const summary = runs.reduce((sum, run) => ({
    ...
    async getPayrollRun(schoolId, runId) {
        ...
        if (!run) {
            throw new localization_1.LocalizedException('payroll.payroll_run_not_found_861c8197', undefined, common_1.HttpStatus.NOT_FOUND, 'Payroll run not found');
        }
        return run;
    }
    async createPayrollRun(user, dto) {
        ...
        if (activeSalaries.length === 0) {
            throw new localization_1.LocalizedException('payroll.add_at_least_one_active_staff_salary_before_creating_payroll_8e71f754', undefined, undefined, 'Add at least one active staff salary before creating payroll');
        }
        ...
        try {
            ...
            if (this.isUniqueConstraintError(error)) {
                throw new localization_1.LocalizedException('payroll.payroll_already_exists_for_this_month_cdb8a7e1', undefined, undefined, 'Payroll already exists for this month');
            }
            throw error;
        }
    }
    async updatePayrollRunStatus(user, runId, dto) {
        return this.prisma.$transaction(async (tx) => {
            const run = await tx.payrollRun.findFirst({
                where: { id: runId, schoolId: dto.schoolId },
            });
            throw new localization_1.LocalizedException('payroll.payroll_run_not_found_861c8197', undefined, common_1.HttpStatus.NOT_FOUND, 'Payroll run not found');
            throw new localization_1.LocalizedException('payroll.paid_payroll_runs_cannot_be_changed_0218fe6f', undefined, undefined, 'Paid payroll runs cannot be changed');
            throw new localization_1.LocalizedException('payroll.cancelled_payroll_runs_cannot_be_changed_0efd96c3', undefined, undefined, 'Cancelled payroll runs cannot be changed');
            if (run.status !== dto.status) {
                ...
                if (!allowedNextStatuses.includes(dto.status)) {
                    throw new localization_1.LocalizedException('payroll.payroll_run_must_move_from_draft_to_approved_before_payment_73789a07', undefined, undefined, 'Payroll run must move from DRAFT to APPROVED before payment');
                }
            }
            const statusData = {
                status: dto.status,
            ...
            if (dto.status === 'PAID') {
                ...
                const counts = entryStatuses.reduce((sum, row) => ({ ...sum, [row.status]: row._count._all }), {});
                throw new localization_1.LocalizedException('payroll.approve_the_payroll_run_before_marking_it_paid_198b4953', undefined, undefined, 'Approve the payroll run before marking it paid');
                throw new localization_1.LocalizedException('payroll.payroll_has_no_payable_entries_to_mark_as_paid_c03e05c2', undefined, undefined, 'Payroll has no payable entries to mark as paid');
    ...
    async updatePayrollEntryStatus(user, entryId, dto) {
        return this.prisma.$transaction(async (tx) => {
            const entry = await tx.payrollEntry.findFirst({
                where: { id: entryId, schoolId: dto.schoolId },
                include: { run: { select: { status: true } } },
            });
            throw new localization_1.LocalizedException('payroll.payroll_entry_not_found_4bba9b4b', undefined, common_1.HttpStatus.NOT_FOUND, 'Payroll entry not found');
            if (entry.run.status === 'PAID' || entry.run.status === 'CANCELLED') {
                throw new localization_1.LocalizedException('payroll.entries_cannot_be_changed_after_the_payroll_run_is_final_25b100e3', undefined, undefined, 'Entries cannot be changed after the payroll run is final');
            }
            if (entry.status === 'PAID' && dto.status !== 'PAID') {
                throw new localization_1.LocalizedException('payroll.paid_payroll_entries_cannot_be_reopened_c95a43d6', undefined, undefined, 'Paid payroll entries cannot be reopened');
            }
            if (dto.status === 'PAID' && entry.run.status !== 'APPROVED') {
                throw new localization_1.LocalizedException('payroll.approve_the_payroll_run_before_paying_staff_entries_63a39624', undefined, undefined, 'Approve the payroll run before paying staff entries');
            }
    ...
    getPayrollStaffRoles() {
        return [role_enum_1.Role.ADMIN, role_enum_1.Role.IT_MANAGER, role_enum_1.Role.REGISTRAR, role_enum_1.Role.TEACHER, role_enum_1.Role.FINANCE];
    }
    ...
    async refreshPayrollRunTotals(tx, runId) {
        ...
        const payableEntries = entries.filter((e) => e.status !== 'HELD');
    ...
    async notifyFinanceForMissingPayrollRun(school, periodMonth, periodYear, calendarType) {
        const financeUsers = await this.prisma.user.findMany({
            where: { schoolId: school.id, role: role_enum_1.Role.FINANCE, isActive: true, deletedAt: null },
            select: { id: true },
        });
    ...
    async notifyFinanceForPayrollRunDue(run, daysBefore) {
        const financeUsers = await this.prisma.user.findMany({
            where: { schoolId: run.schoolId, role: role_enum_1.Role.FINANCE, isActive: true, deletedAt: null },
            select: { id: true },
        });
        if (financeUsers.length === 0)
            return;
        const schoolCalendarType = await this.getSchoolCalendarType(run.schoolId);
        const calendarType = run.periodCalendarType === 'ETHIOPIAN' || run.periodCalendarType === 'GREGORIAN'
...
], PayrollService.prototype, "notifyFinanceForUpcomingPayrollPayments", null);
__decorate([
    (0, schedule_1.Cron)(schedule_1.CronExpression.EVERY_DAY_AT_8AM),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], PayrollService.prototype, "notifyFinanceToCreateCurrentPayrollRun", null);
