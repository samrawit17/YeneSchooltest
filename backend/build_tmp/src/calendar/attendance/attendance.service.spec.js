...
describe('AttendanceService ownership guards', () => {
    const createService = (prisma, schoolSettings = {}) => new attendance_service_1.AttendanceService(prisma, { getSetting: jest.fn().mockResolvedValue(null), ...schoolSettings }, { emit: jest.fn() });
    it('requires parent-child attendance access to stay inside the parent school', async () => {
        ...
        await expect(service.getStudentAttendanceSummary({ id: 'parent-user-1', role: role_enum_1.Role.PARENT, schoolId: 'school-1', name: 'Parent User' }, 'student-user-1')).rejects.toBeInstanceOf(common_1.ForbiddenException);
    ...
    it('filters student self attendance through same-school sessions', async () => {
        ...
        const service = createService(prisma);
        await service.getMyAttendance({ id: 'student-user-1', role: role_enum_1.Role.STUDENT, schoolId: 'school-1', name: 'Student User' }, {});
        expect(prisma.attendanceRecord.findMany).toHaveBeenCalledWith(expect.objectContaining({
            where: {
    ...
    it('uses local-day ranges for admin dashboard stats and recent absences', async () => {
        ...
        const service = createService(prisma);
        const result = await service.getAdminDashboard({ id: 'admin-user-1', role: role_enum_1.Role.ADMIN, schoolId: 'school-1' }, '2026-05-15');
        expect(prisma.attendanceSession.findMany).toHaveBeenNthCalledWith(1, expect.objectContaining({
