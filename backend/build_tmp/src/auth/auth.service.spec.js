...
describe('AuthService password reset workflow', () => {
    const makeService = () => {
        ...
        const service = new auth_service_1.AuthService(prisma, {}, credentialService, notificationService, eventBus);
    ...
    it('notifies active school admins and IT managers for a username reset request', async () => {
        ...
        expect(notificationService.createBulkNotifications).toHaveBeenCalledWith(expect.objectContaining({
            schoolId: 'school-1',
    ...
    it('lets an IT manager reset a same-school user password', async () => {
        ...
        const result = await service.adminResetUserPassword('student-user-1', 'it-user-1', 'school-1', role_enum_1.Role.IT_MANAGER);
    ...
    it('accepts a simple admin-entered temporary password and forces reset', async () => {
        ...
        prisma.user.update.mockResolvedValue({});
        const result = await service.adminResetUserPassword('student-user-1', 'admin-user-1', 'school-1', role_enum_1.Role.ADMIN, '12345678');
        ...
        expect(credentialService.hashPassword).toHaveBeenCalledWith('12345678');
        expect(prisma.user.update).toHaveBeenCalledWith({
            where: { id: 'student-user-1' },
            data: {
                password: 'hashed-password',
    ...
    it('rejects admin-entered temporary passwords shorter than 8 characters', async () => {
        ...
        await expect(service.adminResetUserPassword('student-user-1', 'admin-user-1', 'school-1', role_enum_1.Role.ADMIN, '1234567')).rejects.toThrow('Temporary password must be at least 8 characters');
    ...
    it('blocks cross-school password resets', async () => {
        ...
        await expect(service.adminResetUserPassword('student-user-1', 'admin-user-1', 'school-1', role_enum_1.Role.ADMIN)).rejects.toBeInstanceOf(common_1.ForbiddenException);
        expect(prisma.user.update).not.toHaveBeenCalled();
    });
});
