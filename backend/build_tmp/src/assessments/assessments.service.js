...
let AssessmentsService = class AssessmentsService {
    ...
    cacheService;
    eventBus;
    constructor(prisma, cacheService, eventBus) {
        this.prisma = prisma;
    ...
    async invalidateAssessmentGradeCaches(schoolId, teacherIds = []) {
        const uniqueTeacherIds = Array.from(new Set(teacherIds.filter((teacherId) => Boolean(teacherId))));
        await Promise.all([
    ...
    async getAssessmentAffectedTeacherIds(schoolId, assessmentId) {
    ...
    async resolveChildStudentForParent(parentUserId, childIdOrUserId, schoolId) {
        const parentProfile = await this.prisma.parentProfile.findFirst({
            where: { userId: parentUserId, schoolId },
            select: { id: true },
        });
        if (!parentProfile) {
            throw new localization_1.LocalizedException('assessments.parent_profile_not_found_ad089d27', undefined, common_1.HttpStatus.NOT_FOUND, 'Parent profile not found');
        }
        ...
        if (!studentProfile) {
            throw new localization_1.LocalizedException('assessments.student_not_found_2525e0b2', undefined, common_1.HttpStatus.NOT_FOUND, 'Student not found');
        }
        ...
        if (!parentStudent) {
            throw new localization_1.LocalizedException('assessments.you_are_not_linked_to_this_student_49797e72', undefined, common_1.HttpStatus.FORBIDDEN, 'You are not linked to this student');
        }
    ...
    async validateAssessmentContext(schoolId, dto) {
        ...
        if (!academicYear) {
            throw new localization_1.LocalizedException('assessments.academic_year_not_found_561c725b', undefined, common_1.HttpStatus.NOT_FOUND, 'Academic year not found');
        }
        if (dto.termId) {
            ...
            if (!term) {
                throw new localization_1.LocalizedException('assessments.term_not_found_for_academic_year_1a90ff3d', undefined, common_1.HttpStatus.NOT_FOUND, 'Term not found for academic year');
            }
        }
        if (new Date(dto.endDate) < new Date(dto.startDate)) {
            throw new localization_1.LocalizedException('assessments.end_date_cannot_be_before_start_date_71f9ba01', undefined, undefined, 'End date cannot be before start date');
        }
    }
    assessmentSubjectTargetKey(subject) {
        return `${subject.classId}:${subject.sectionId ?? 'all'}:${subject.subjectId}`;
    }
    async assertNoDuplicateAssessmentTargets(assessment, subjects) {
        const requestTargets = new Set();
        for (const subject of subjects) {
            const key = this.assessmentSubjectTargetKey(subject);
            if (requestTargets.has(key)) {
                throw new localization_1.LocalizedException('assessments.the_same_class_section_and_subject_was_selected_more_than_on_93984c06', undefined, undefined, 'The same class, section, and subject was selected more than once');
            }
        ...
        const conflicts = await this.prisma.assessmentSubject.findMany({
            where: {
                ...
                assessment: {
                    ...
                    type: String(assessment.type).toUpperCase(),
                    status: { notIn: [client_1.AssessmentStatus.COMPLETED, client_1.AssessmentStatus.LOCKED] },
                },
            },
            include: {
        ...
        const conflict = conflicts.find((item) => requestTargets.has(this.assessmentSubjectTargetKey(item)));
        if (conflict) {
            const sectionName = conflict.section?.name
                ? ` (${conflict.section.name})`
                : '';
            throw new localization_1.LocalizedException('assessments.assessment_already_exists_for_complete_or_lock_before_creati_ccc6bce9', undefined, undefined, '${String(conflict.assessment.type).toUpperCase()} assessment already exists for ${conflict.class.name}${sectionName} - ${conflict.subject.name}. Complete or lock "${conflict.assessment.title}" before creating another one.');
        }
    }
    async resolveTeacherAssignment(teacherId, academicYearId, subject) {
        ...
        if (classSubject)
            return teacherId;
        throw new localization_1.LocalizedException('assessments.you_are_not_assigned_to_one_or_more_selected_subjects_cb516b2f', undefined, common_1.HttpStatus.FORBIDDEN, 'You are not assigned to one or more selected subjects');
    }
    async ensureAssessmentWriteAccess(schoolId, userId, role, assessmentId) {
        ...
        if (!assessment) {
            throw new localization_1.LocalizedException('assessments.assessment_not_found_c5381dbc', undefined, common_1.HttpStatus.NOT_FOUND, 'Assessment not found');
        }
        if (assessment.status === client_1.AssessmentStatus.LOCKED) {
            throw new localization_1.LocalizedException('assessments.assessment_is_locked_99cc7fe3', undefined, common_1.HttpStatus.FORBIDDEN, 'Assessment is locked');
        }
        if (role === 'TEACHER' &&
            (assessment.createdBy !== userId ||
                !TEACHER_MANAGED_ASSESSMENT_TYPES.has(String(assessment.type).toUpperCase()))) {
            throw new localization_1.LocalizedException('assessments.teachers_can_only_manage_their_own_quiz_and_test_assessments_32a75b21', undefined, common_1.HttpStatus.FORBIDDEN, 'Teachers can only manage their own quiz and test assessments');
        }
        return assessment;
    }
    async ensureTeacherCanScore(teacherId, assessmentSubjectId, schoolId) {
        ...
        if (!assessmentSubject) {
            throw new localization_1.LocalizedException('assessments.assessment_subject_not_found_6f6fdaca', undefined, common_1.HttpStatus.NOT_FOUND, 'Assessment subject not found');
        }
    ...
    async syncSubjectGradeForStudent(assessmentSubjectId, studentId) {
        ...
        if (!resolvedSectionId) {
            throw new localization_1.LocalizedException('assessments.student_section_could_not_be_resolved_for_subject_grade_sync_359cc52f', undefined, undefined, 'Student section could not be resolved for subject grade sync');
        }
    ...
    async createAssessmentSubjects(assessment, subjects, actorId, role) {
        ...
        for (const item of subjects) {
            if (!subjectSet.has(item.subjectId)) {
                throw new localization_1.LocalizedException('assessments.subject_not_found_562e5a84', undefined, common_1.HttpStatus.NOT_FOUND, 'Subject not found');
            }
            const classRecord = classMap.get(item.classId);
            if (!classRecord) {
                throw new localization_1.LocalizedException('assessments.class_not_found_7fd09a97', undefined, common_1.HttpStatus.NOT_FOUND, 'Class not found');
            }
        }
        let sectionsFound = [];
    ...
    assessmentSubjectScoreKey(input) {
        return [
    ...
    async attachEffectiveScoreCountsToAssessments(schoolId, assessments) {
        const targets = assessments.flatMap((assessment) => (assessment.subjects || []).map((subject) => ({
        ...
        if (targets.length === 0)
            return assessments;
        const componentCodes = Array.from(new Set(targets.map((target) => target.assessmentType)));
        ...
        const gradeScoreCounts = new Map();
    ...
    async createAssessment(schoolId, userId, role, dto) {
        if (role === 'TEACHER' &&
            ...
            throw new localization_1.LocalizedException('assessments.teachers_can_only_create_quizzes_and_tests_d737c661', undefined, common_1.HttpStatus.FORBIDDEN, 'Teachers can only create quizzes and tests');
        }
        ...
        if (dto.subjects?.length) {
            const createdSubjects = await this.createAssessmentSubjects(assessment, dto.subjects, userId, role);
            const affectedTeacherIds = await this.getAssessmentAffectedTeacherIds(schoolId, assessment.id);
    ...
    async getAssessmentById(schoolId, id) {
        ...
        const [withCounts] = await this.attachEffectiveScoreCountsToAssessments(schoolId, [assessment]);
        return withCounts;
    }
    async updateAssessment(schoolId, userId, role, id, dto) {
        const assessment = await this.ensureAssessmentWriteAccess(schoolId, userId, role, id);
        const startDate = dto.startDate
            ? new Date(dto.startDate)
            : assessment.startDate;
        const endDate = dto.endDate ? new Date(dto.endDate) : assessment.endDate;
        if (Number.isNaN(startDate.getTime()) || Number.isNaN(endDate.getTime())) {
            throw new localization_1.LocalizedException('assessments.assessment_dates_are_invalid_6f7d6517', undefined, undefined, 'Assessment dates are invalid');
        }
        ...
        const affectedTeacherIds = await this.getAssessmentAffectedTeacherIds(schoolId, updated.id);
        await this.invalidateAssessmentGradeCaches(schoolId, affectedTeacherIds);
    ...
    async saveScores(userId, role, schoolId, assessmentSubjectId, dto) {
        ...
        if (assessmentSubject.assessment.status === client_1.AssessmentStatus.LOCKED &&
            ...
            throw new localization_1.LocalizedException('assessments.assessment_scores_are_locked_121cf32e', undefined, common_1.HttpStatus.FORBIDDEN, 'Assessment scores are locked');
        }
        const weights = await this.getWeightMap(schoolId);
        const effectiveMaxScore = this.getEffectiveMaxScore(assessmentSubject.maxScore, assessmentSubject.assessment.type, weights);
        const academicYearRecord = await this.prisma.academicYear.findUnique({
        ...
        for (const score of dto.scores) {
            if (!validStudentIds.has(score.studentId)) {
                throw new localization_1.LocalizedException('assessments.one_or_more_students_are_not_in_the_class_a58fd7b8', undefined, undefined, 'One or more students are not in the class');
            }
            if (score.score !== undefined &&
                score.score !== null &&
                score.score > effectiveMaxScore) {
                throw new localization_1.LocalizedException('assessments.score_cannot_exceed_44bbf131', undefined, undefined, 'Score cannot exceed ${effectiveMaxScore}');
            }
        }
    ...
    async lockAssessment(schoolId, assessmentId) {
        ...
        const teacherIds = await this.getAssessmentAffectedTeacherIds(schoolId, assessmentId);
    ...
    async updateWeights(schoolId, dto) {
        ...
        if (Math.round(total * 100) / 100 !== 100) {
            throw new localization_1.LocalizedException('assessments.assessment_weights_must_total_100_72fa5a62', undefined, undefined, 'Assessment weights must total 100');
        }
    ...
    async getParentUpcoming(parentUserId, childId, schoolId, academicYearId) {
        const studentId = await this.resolveChildStudentForParent(parentUserId, childId, schoolId);
