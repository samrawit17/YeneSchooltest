...
let SeatingService = SeatingService_1 = class SeatingService {
    ...
    async createSeatingPlanByExamType(schoolId, userId, examType, dto) {
        if (!this.isSupportedExamType(examType)) {
            throw new localization_1.LocalizedException('exams.exam_seating_is_only_supported_for_mid_and_final_exams_2f14bc61', undefined, undefined, 'Exam seating is only supported for mid and final exams');
        }
        if (dto.mode === seating_dto_1.SeatingMode.GRADE_RANGE) {
            if (!dto.toGrade) {
                throw new localization_1.LocalizedException('exams.tograde_is_required_for_grade_range_mode_5cdac510', undefined, undefined, 'toGrade is required for grade range mode');
            }
            if (dto.toGrade < dto.fromGrade) {
                throw new localization_1.LocalizedException('exams.tograde_must_be_greater_than_or_equal_to_fromgrade_df110689', undefined, undefined, 'toGrade must be greater than or equal to fromGrade');
            }
        }
        ...
        if (existingPlan) {
            throw new localization_1.LocalizedException('exams.a_seating_plan_already_exists_for_this_exam_type_delete_it_f_d9458c6c', undefined, common_2.HttpStatus.CONFLICT, 'A seating plan already exists for this exam type. Delete it first to create a new one.');
        }
    ...
    async deleteSeatingStudents(schoolId, planId) {
        ...
        if (!plan) {
            throw new localization_1.LocalizedException('exams.seating_plan_not_found_2fe7dbd4', undefined, common_2.HttpStatus.NOT_FOUND, 'Seating plan not found');
        }
    ...
    async createSeatingPlan(schoolId, userId, examId, dto) {
        ...
        if (!exam) {
            throw new localization_1.LocalizedException('exams.exam_not_found_or_does_not_belong_to_your_school_af43fd8c', undefined, common_2.HttpStatus.NOT_FOUND, 'Exam not found or does not belong to your school');
        }
        if (dto.mode === seating_dto_1.SeatingMode.GRADE_RANGE) {
            if (!dto.toGrade) {
                throw new localization_1.LocalizedException('exams.tograde_is_required_for_grade_range_mode_5cdac510', undefined, undefined, 'toGrade is required for grade range mode');
        ...
        if (existingPlan) {
            throw new localization_1.LocalizedException('exams.a_seating_plan_already_exists_for_this_exam_delete_it_first__7d19a3f9', undefined, undefined, 'A seating plan already exists for this exam. Delete it first to create a new one.');
        }
        const plan = await this.prisma.examSeatingPlan.create({
            data: {
                examId,
    ...
    async generateSeating(schoolId, planId) {
        ...
        if (!plan) {
            throw new localization_1.LocalizedException('exams.seating_plan_not_found_or_does_not_belong_to_your_school_063fd976', undefined, common_2.HttpStatus.NOT_FOUND, 'Seating plan not found or does not belong to your school');
        }
        if (!this.isSupportedExamType(plan.examType)) {
            throw new localization_1.LocalizedException('exams.only_mid_and_final_exams_can_use_exam_seating_6a9d5f57', undefined, undefined, 'Only mid and final exams can use exam seating');
        }
        ...
        if (existingAssignments.length > 0 &&
            ...
            throw new localization_1.LocalizedException('exams.seating_has_already_been_generated_for_this_plan_delete_the__109e7094', undefined, undefined, 'Seating has already been generated for this plan. Delete the existing seating first to regenerate.');
        }
        ...
        if (!activeYear) {
            throw new localization_1.LocalizedException('exams.no_active_academic_year_found_47ece9b3', undefined, undefined, 'No active academic year found');
        }
        ...
        if (studentClasses.length === 0) {
            throw new localization_1.LocalizedException('exams.no_students_found_in_the_selected_grade_range_students_must__87e42636', undefined, undefined, 'No students found in the selected grade range. Students must be enrolled in the active academic year (\' + activeYear.name + \'). Please enroll students first.');
        }
        ...
        if (sections.length === 0) {
            throw new localization_1.LocalizedException('exams.no_sections_available_for_the_selected_grade_range_2dd7d7c4', undefined, undefined, 'No sections available for the selected grade range');
        }
        if (sections.length < numSections) {
            throw new localization_1.LocalizedException('exams.not_enough_existing_sections_for_this_seating_plan_required__1f5082f8', undefined, undefined, 'Not enough existing sections for this seating plan. Required: ${numSections}, available: ${sections.length}. Create the academic sections first instead of generating temporary exam sections.');
        }
    ...
    async orderStudentsByMidResult(schoolId, students, academicYear, termId) {
        ...
        if (rankedStudents.every((row) => row.rankScore === Number.NEGATIVE_INFINITY)) {
            throw new localization_1.LocalizedException('exams.no_mid_exam_results_were_found_for_these_students_so_perform_3cab36a4', undefined, undefined, 'No mid exam results were found for these students, so performance-based final seating cannot be generated yet');
        }
    ...
    async orderStudentsByPreviousFinalResult(schoolId, students, academicYearId, academicYear, currentTermId) {
        ...
        const rankedStudents = students
            ...
            .sort((a, b) => b.rankScore - a.rankScore || a.student.name.localeCompare(b.student.name));
        if (rankedStudents.every((row) => row.rankScore === Number.NEGATIVE_INFINITY)) {
            throw new localization_1.LocalizedException('exams.no_previous_final_results_were_found_for_these_students_so_p_895db20f', undefined, undefined, 'No previous final results were found for these students, so performance-based mid seating cannot be generated yet');
        }
        return rankedStudents.map((row) => row.student);
    }
};
