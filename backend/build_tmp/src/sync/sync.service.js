...
let SyncService = SyncService_1 = class SyncService {
    ...
    async syncAttendance(dto, user, deviceId) {
        ...
        if (!schoolId || !actorId) {
            throw new localization_1.LocalizedException('sync.authenticated_school_and_user_are_required_for_sync_27b57cd1', undefined, undefined, 'Authenticated school and user are required for sync');
        }
        try {
            switch (operation) {
                case 'create':
                ...
                default:
                    throw new localization_1.LocalizedException('sync.invalid_operation_c227b161', undefined, undefined, 'Invalid operation: ${operation}');
            }
        }
        catch (error) {
    ...
    async handleCreateAttendance(data) {
        ...
        if (existing) {
            ...
            if (existingModified > localModifiedTime) {
                await this.recordConflict(data.schoolId, 'attendance', `${data.studentId}_${data.date}`, 'update_update', data, existing, data.deviceId);
                return {
                    success: false,
    ...
    async handleUpdateAttendance(data) {
        ...
        const localModifiedTime = new Date(data.localModified).getTime();
        if (existingModified > localModifiedTime) {
            await this.recordConflict(data.schoolId, 'attendance', data.entityId, 'update_update', data, existing, data.deviceId);
            return {
                success: false,
                serverId: existing.id,
                version: 1,
    ...
    async getStudentsForOffline(user, classIds, sectionIds) {
        if (!user?.schoolId) {
            throw new localization_1.LocalizedException('sync.authenticated_school_is_required_e729adbd', undefined, undefined, 'Authenticated school is required');
        }
    ...
    async validateAttendanceScope(schoolId, studentId, classId, sectionId) {
        if (!studentId || !classId || !sectionId) {
            throw new localization_1.LocalizedException('sync.studentid_classid_and_sectionid_are_required_25054980', undefined, undefined, 'studentId, classId, and sectionId are required');
        }
        ...
        if (!enrollment) {
            throw new localization_1.LocalizedException('sync.student_is_not_enrolled_in_this_class_section_for_your_schoo_1d425733', undefined, undefined, 'Student is not enrolled in this class/section for your school');
        }
    }
    normalizeAttendanceStatus(status) {
        const normalized = String(status || '').trim().toUpperCase();
        if (!['PRESENT', 'ABSENT', 'LATE', 'EXCUSED'].includes(normalized)) {
            throw new localization_1.LocalizedException('sync.invalid_attendance_status_7ce25012', undefined, undefined, 'Invalid attendance status');
        }
        return normalized;
    }
    async getConflicts(schoolId) {
    ...
    async recordConflict(schoolId, entity, entityId, conflictType, localData, serverData, deviceId) {
    ...
    async resolveConflict(id, resolution, data, resolvedBy) {
        const conflict = await this.prisma.syncConflict.findUnique({
            where: { id },
        });
        if (!conflict) {
            throw new localization_1.LocalizedException('sync.conflict_not_found_2889071e', undefined, undefined, 'Conflict not found');
        }
