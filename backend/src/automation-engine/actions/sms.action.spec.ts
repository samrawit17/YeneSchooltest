...
describe('SmsAction', () => {
  ...
  it('returns success with queued message when "to" is provided', async () => {
    const result = await action.execute(
      ...
      { to: '+251911111111', message: 'Hello {{name}}' },
    );
  ...
  it('falls back to event payload phone when "to" is missing', async () => {
    const result = await action.execute(
      { eventType: 'test', payload: { phone: '+251922222222' }, schoolId: 's1', timestamp: new Date() },
      { message: 'Alert' },
    );
  ...
  it('compiles template variables from payload', async () => {
    const result = await action.execute(
      { eventType: 'test', payload: { name: 'Abebe', grade: 'A' }, schoolId: 's1', timestamp: new Date() },
      { to: '+251911111111', message: 'Hi {{name}}, your grade is {{grade}}' },
    );
