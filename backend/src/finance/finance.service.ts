...
export class FinanceService {
  ...
  // ========================================================
  // PAYROLL METHODS (FINANCE)
  // ========================================================

  async getPayrollsForFinance(
    schoolId: string,
  ...
  async processPaymentToBank(schoolId: string, payrollId: string, userId: string) {
    const payroll = await this.prisma.payroll.findFirst({
      where: { id: payrollId, schoolId },
    });
    ...
    if (payroll.status !== 'SUBMITTED') {
      throw new Error('Payroll must be SUBMITTED by HR before processing payment');
    }
  ...
  async markPayrollAsPaidFinance(
    schoolId: string,
    payrollId: string,
    userId: string,
    ...
    if (!['PROCESSED', 'PENDING_PAYMENT'].includes(payroll.status)) {
  ...
  async getPayrollReports(
    ...
    const totalEmployees = payrolls.reduce(
      (sum, p) => sum + (p.payrollItems?.length || 0),
      0,
    );
    ...
    return {
      ...
      summary: {
        ...
        totalEmployees,
        averagePayroll: payrolls.length > 0 ? totalAmount / payrolls.length : 0,
      },
    };
  }
}
