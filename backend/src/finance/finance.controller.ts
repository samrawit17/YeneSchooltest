...
export class FinanceController {
  ...
  /**
   * Get payroll list - Finance view (includes pending payment status)
   */
  @Get('payroll')
  @Permissions('payroll:view_all')
  async getPayrollsFinance(
    @Request() req: any,
    @Query('page') page?: number,
    @Query('limit') limit?: number,
    @Query('status') status?: string,
  ) {
    const result = await this.financeService.getPayrollsForFinance(
      req.user.schoolId,
  ...
  @Get('payroll/:id')
  @Permissions('payroll:view_all')
  async getPayrollByIdFinance(
    @Request() req: any,
    @Param('id') id: string,
  ) {
    const result = await this.financeService.getPayrollById(req.user.schoolId, id);
    return { success: true, data: result };
  }

  /**
   * Process payment - mark as pending payment (Finance ready to process to bank)
   */
  @Post('payroll/:id/process-payment')
  @Permissions('payroll:process_payment')
  async processPaymentToBank(
    @Request() req: any,
    @Param('id') id: string,
  ) {
    const result = await this.financeService.processPaymentToBank(
      req.user.schoolId,
      id,
      req.user.id,
    );
  ...
  /**
   * Mark as paid - with payment reference from bank
   */
  ...
  async markPayrollAsPaid(
    ...
    const result = await this.financeService.markPayrollAsPaidFinance(
      req.user.schoolId,
      id,
      req.user.id,
  ...
  /**
   * Get payroll reports for finance
   */
  @Get('payroll/reports')
  ...
  async getPayrollReports(
    ...
    const result = await this.financeService.getPayrollReports(
      req.user.schoolId,
      startDate,
      endDate,
    );
