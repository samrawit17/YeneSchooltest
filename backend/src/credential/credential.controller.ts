...
export class CredentialController {
  ...
  async createStudents(
    ...
    const submittedFaydaNumbers = new Set<string>();
    for (const student of students) {
      const faydaNumber = String(student.faydaNumber || '').replace(/\D/g, '');
      if (!faydaNumber) {
        throw new BadRequestException('Fayda number is required for every student');
      }
      if (!/^\d{12}$/.test(faydaNumber)) {
        throw new BadRequestException('Fayda number must be 12 digits');
      }
      if (submittedFaydaNumbers.has(faydaNumber)) {
        throw new BadRequestException(`Duplicate Fayda number: ${faydaNumber}`);
      }
    ...
    if (existingFayda?.faydaNumber) {
      throw new BadRequestException(
        `Fayda number already exists: ${existingFayda.faydaNumber}`,
      );
    }
    ...
    const normalizeStudentStream = (stream?: string | null, grade?: number | null) => {
    ...
    for (const student of students) {
      ...
      const createdStudent = await this.prismaService.$transaction(
        async (tx) => {
          ...
          let assignedStream: string | null = null;
          ...
          if (student.classId) {
            ...
            if (student.sectionId) {
              ...
              if (currentEnrollment >= selectedSection.capacity) {
                throw new BadRequestException(
                  `Section ${selectedSection.name} is full (${currentEnrollment}/${selectedSection.capacity}). Choose another section or increase the section capacity.`,
                );
              }
          ...
          if (parentName) {
            let parentUser = parentPhone
              ? await tx.user.findFirst({
                  where: { schoolId, phone: parentPhone, role: Role.PARENT },
                })
              : null;
